...
const verifyPayment = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: "Payment sent for admin verification" });
  } catch (error) {
    console.error(error);
...
const handleDepositAction = async (req, res) => {
  ...
  try {
    ...
    if (action === "APPROVE") {
      ...
      await pool.query(
        "UPDATE deposit_requests SET status = 'REJECTED' WHERE id = $1",
        [deposit_id],
      );
    }
    ...
    res
      .status(200)
      .json({ success: true, message: `Deposit successfully ${action}D` });
  } catch (error) {
